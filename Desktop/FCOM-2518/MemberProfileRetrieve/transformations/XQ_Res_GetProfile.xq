...
declare function xf:XQ_Res_GetProfile($QuerybyExampleFullMemberResponse as element(*),$dvmTable as element(*),$CitizendvmTable as element(*),$SiebelResponse as element(*),$HobbiesDvm as element(*),$DestinationsWithoutSpace as xs:string,$VacationsWithoutSpace as xs:string)
...
for $var in (1 to fn:count($Contact/ayr:ListOfPersonalAddress/ayr:PersonalAddress))
                       ...
                       let $addressLine := $path/ayr:PersonalStreetAddress/text()
                        ...
                        (: FCOM-1653 : $countryName moved earlier as we need it right away :)
                        let $countryName := $path/ayr:PersonalCountry/text()
                        
                        (:
                           As per FCOM-1653 we don't make "any" changes to MemberProfileRetrieve.proxy (and per
                           extension, not to XQ_Res_GetProfile.xq either... but we should still prevent any new
                           values in $path/ayr:PersonalStreetAddress2 for Korea, Canada, China, India and New
                           Zealand from showing up as descriptions, which would otherwise happen. So the check
                           for JP/UK _must_ be added or we'll see a functional change.
                        :)
                        let $Description :=
                            if (exists($countryName) and dvm:lookupValue($dvmTable, 'CountryName', $countryName, 'CountryCode', "") = ("JP", "GB")) then
                                $path/ayr:PersonalStreetAddress2/text()
                            else
                                ""
                        ...
                        (:
                           As per FCOM-1653 we don't make "any" changes to MemberProfileRetrieve.proxy (and per
                           extension, not to XQ_Res_GetProfile.xq either... but we should still prevent any new
                           values in $path/ayr:PersonalStreetAddress3 for Korea, Canada, China, India and New
                           Zealand from showing up as descriptions, which would otherwise happen. So the check
                           for KO/CA/CN/IN/NZ _must_ be added or we'll see a functional change.
                        :)
                        let $County :=
                            if (empty($countryName) or not(dvm:lookupValue($dvmTable, 'CountryName', $countryName, 'CountryCode', "") = ("KP", "KR", "CA", "CN", "IN", "NZ"))) then
                            	$path/ayr:PersonalStreetAddress3/text()
                            else
	                            ""
...
<com:CustomField KeyValue = "8">{if ($Contact/ayr:SuppresseMailWeeklyOffers = "N") then
                                                                              ("TRUE")
                                                                          else 
                                                                              "FALSE" }</com:CustomField>
<com:CustomField KeyValue = "9">{if ($LoyMember/ayr:ReceivePartnerPromotionFlag = "Y") then
                                                                              ("TRUE")
                                                                          else 
                                                                              "FALSE" }</com:CustomField>
...
xf:XQ_Res_GetProfile($QuerybyExampleFullMemberResponse,$dvmTable,$CitizendvmTable,$SiebelResponse,$HobbiesDvm,$DestinationsWithoutSpace,$VacationsWithoutSpace)