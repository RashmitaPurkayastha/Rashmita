...
declare function xf:Convert_SiebelDateTime_To_YYYY-MM-DD($SourceDate as xs:string)
    as xs:string {
    if (fn:string-length($SourceDate) > 0) then
   		let $dateString := substring($SourceDate, 1, 10)
   		let $retValue := concat(substring($dateString, 7, 4), '-', substring($dateString, 1, 2), '-', substring($dateString, 4, 2))
		return $retValue
	else
		string('')
};

declare function xf:XQ_Res_IdentifyResponse($body as element(), $LangID as xs:string, $DateOfBirth as xs:string, $Country as xs:string, $PreferredMailingAddress as xs:string, $AYTierStartDate as xs:string, $AYTierEndDate as xs:string)
    as element(mem:IdentifyResponse) {
<mem:IdentifyResponse>
	<com:IdentifyRS>
    	<com:Success/>
        ...
        <com:TierPoints>{data($body/asi:QueryByExampleMemberResponse/SiebelMessage/ayr:ListOfAyresMemberInformation-Short/ayr:LoyMember/ayr:Point1Value)}</com:TierPoints>
        ...
        {
        if (data($PreferredMailingAddress) eq "Home") then (
...
xf:XQ_Res_IdentifyResponse($body, $LangID, $DateOfBirth, $Country, $PreferredMailingAddress, $AYTierStartDate, $AYTierEndDate)